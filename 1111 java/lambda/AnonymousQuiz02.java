...
public class AnonymousQuiz02 {
	public static void main(String[] args) {
		...
		// 익명 클래스 구현
		MaxReturn maxReturnTwo = new MaxReturn() {
			@Override
			public int max(int n1, int n2) {
				return (n1 > n2) ? n1 : n2;
			}
		...
		int maxTwo = maxReturnTwo.max(5, 2);
		
		// 출력
		System.out.println("[익명 클래스 : 참조변수 이용] 큰 값 >> " + maxTwo);
		...
		// 출력
		System.out.println("[익명 클래스 : Lambda 이용] 큰 값 >> " 
							+ lambda.max(5, 2));
		
		// [문제 3-2] 익명 클래스 : System.out.println() 메소드 내 익명 클래스 구현 ==
		System.out.println("[익명 클래스 : 바로 사용] 큰 값 >> " 
							+ new MaxReturn() {
								@Override
								public int max(int n1, int n2) {
									return (n1 > n2) ? n1 : n2;
		...
		System.out.println("[lambda : 바로 사용] 큰 값 >> " 
							+ (lambda = ((n1, n2) -> (n1 > n2) ? n1 : n2)).max(5, 2));
	}
}